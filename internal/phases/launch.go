...
func RunLaunch(ctx LaunchContext) (*LaunchResult, error) {
	...
	if needsBuild(profile.Primary) {
		...
		buildErr := ui.RunWithSpinner("Building", func() error {
			...
			// For C/C++ projects, use the cascade-aware builder so cmake can
			// auto-install missing libraries (fixes Neovim, Redis, etc.)
			if profile.Primary == detection.TechCpp || profile.Primary == detection.TechC {
				var cascade *pkgmanager.Cascade
				if ctx.PkgInfo != nil {
		...
		installErr := ui.RunWithSpinner("Installing globally", func() error {
			...
			if profile.Primary == detection.TechNode {
				...
				if !hasValidPackageName(profile.WorkingDir) {
					if log != nil {
						log.Write("[install] package.json missing or has no 'name' field — trying npm link instead")
					}
					// Fallback: npm link doesn't require the name field
					linkErr := runIn(profile.WorkingDir, safeLog(), extraEnv, "npm", "link")
					if linkErr != nil {
						// Manual symlink fallback: find binaries in node_modules/.bin and symlink them
						if log != nil {
							log.Write("[install] npm link also failed — trying manual symlink from node_modules/.bin")
						}
						symlinkNodeBins(profile.WorkingDir, environment, safeLog())
					}
			...
			// Final fallback for Node: if npm install -g failed, try npm link
			if err != nil && profile.Primary == detection.TechNode {
				if log != nil {
					log.Write("[install] npm install -g failed — falling back to npm link")
				}
				linkErr := runIn(profile.WorkingDir, safeLog(), extraEnv, "npm", "link")
				if linkErr == nil {
					err = nil
...
func launchDotfiles(repoPath string, log pkgmanager.LogWriter) error {
	...
	// Strategy 6: No automatic installer found — offer to render README
	...
	// Look for README to help the user with manual setup
	readmePath := findReadmeFile(repoPath)
	if readmePath != "" {
		shouldRead, _ := ui.Confirm("Would you like to read the README for setup instructions?")
		if shouldRead {
			fmt.Println()
...
func symlinkNodeBins(repoPath string, environment *env.Environment, log pkgmanager.LogWriter) {
	...
	for _, entry := range entries {
		...
		if err := environment.MakeGlobal(entry.Name(), env.LogWriter(log)); err != nil {
			// Try direct symlink as fallback
			target := filepath.Join(environment.BinDir, entry.Name())
			_ = os.Remove(target)
			_ = os.Symlink(source, target)
			if log != nil {
				log(fmt.Sprintf("[node] symlinked %s → %s", source, target))
			}
		}
	}
}
